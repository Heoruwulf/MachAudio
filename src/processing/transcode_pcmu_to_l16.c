...
int transcode_pcmu_to_l16(
    ...
    for (; i < in_len; i += 16) {
        ...
        __m256i const samples_lo_32 =
            _mm256_i32gather_epi32((int const *)PCMU_TO_L16_LUT_32, indices_lo_32, 4);
        __m256i const samples_hi_32 =
            _mm256_i32gather_epi32((int const *)PCMU_TO_L16_LUT_32, indices_hi_32, 4);
...
#else
    // Fallback scalar loop for non-AVX2 targets, compiler will vectorize
    for (; i < in_len; ++i) {
        out_data[i] = PCMU_TO_L16_LUT[in_data[i]];
    }
#endif
